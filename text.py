...
WELCOME_MESSAGE = '''
                \nПривет!👋🏽\
                \n\
                \nНаш сервис дает возможность испытать *новый опыт* в *покупках* и *заработке*, обеспечивая *безопасность* для каждого!\
                \n\
                \nДля начала необходимо *зарегистрироваться*, этот процесс займет меньше минуты!\
                \n\
                \nПожалуйста, выбери роль, в которой будешь выступать на нашей площадке: *(ее всегда можно изменить позже)*\
                \n\
                \n① *Покупатель* - в поисках конкретного товара, хочешь найти байера, который его отправит.\
                \n\
                \n② *Продавец* - хочешь заработать и имеешь возможность выкупа товаров в одной или нескольких странах (в этой роли ты также сможешь заказывать нужные тебе товары).\
                '''
...
CANCEL_COMMAND = '\n\nДля отмены изменений воспользуйся командой */cancel*.'
...
WRONG_COUNTRY_METHOD = '''
                        \nВыбрана роль *продавца*, нам необходимо установить твое местоположение, чтобы отправлять заявки от покупателей.\
                        \nВоспользуйся кнопкой *«Отправить геолокацию»*.\
                        \n\
                        \nПри регистрации с компьютера опция может работать *некорректно* - воспользуйся *телефоном* для прохождения данного этапа регистрации.\
                        '''
...
LOCATION_NEEDED = 'Для завершения регистрации необходимо указать страну пребывания.'
...
#! ЗАПРОСЫ ВВОДА
WHOLE_WORLD = 'Выбери настройки для приема заказов:\n\n*Весь мир* - заявки будут приходить по всем товарам;\n\n*Страна пребывания* - будут приходить заявки по товарам, которые по мнению заказчиков можно найти в стране твоего пребывания, либо если заказчик не знает где можно найти интересующий его товар.'
...
INPUT_OR_CHOOSE_ADDRESS = 'Введи *адрес* для доставки, указав *страну* и *город*, или выбери вариант из списка ниже.' + CANCEL_ORDER_COMMAND

PRODUCT_NAME = 'Укажи *наименование товара* (например: бренд, модель, артикул, если товар - одежда).\nЕсли предоставленных данных будет недостаточно для идентификации товара - заявка может быть отклонена администраторами.' + CANCEL_ORDER_COMMAND
...
PRODUCT_COMMENT = 'Укажи *комментарий к заказу*, воспользуйся командой */skip* для пропуска этого шага.' + CANCEL_ORDER_COMMAND
...
NO_MEDIA_GROUP = 'Пожалуйста, отправляй *по одной фотографии* за раз.'
...
def ask_response_price(name):
    ...
    return f'Укажи *стоимость в рублях*, по которой готов отправить товар ({name}).\nСоветуем принять во внимание возможные колебания валютного курса.' + CANCEL_RESPONSE_COMMAND


def incorrect_price(name):
    name = utils.escape_markdown(name)

    return f'Стоимость не соответствует формату.\nУкажи *стоимость в рублях*, по которой готов отправить заказанный товар ({name}).' + CANCEL_RESPONSE_COMMAND
...
def ask_delivery(name, price):
    ...
    return f'Стоимость *({price} руб.)* за {name} указана с учетом доставки или без?' + CANCEL_RESPONSE_COMMAND
...
def confirm_bank_account(payment_form, bank, account):
    if payment_form == 'sbp':
        ...
        payment_form = 'По номеру банковской карты'
        method = 'Номер банковской карты'
...
def seller_chosen(order_id, seller_username, name, price, delivery, comment):
    ...
    reply_text = f'''
                ...
                \n*Комментарий:* {comment}\
                \n\
                \nДля того, чтобы исполнитель *приступил* к выполнению заказа, необходимо:\
                \nПеревести сумму в размере *{price} руб.* по реквизитам и указать в комментарии к платежу цифру «*{order_id}*»:\
                \n\
                \n*Банк:* Тинькофф\
                \n*Номер карты:* 2200 7001 6681 0308\
                \n*Получатель:* Антон А.\
                \n\
                \nМы проверим поступление платежа, уведомим исполнителя - он получит средства только после полного исполнения своих обязательств.\
                \n\
...
def payment_verified_seller(order_id, name):
    ...
    return f'Платеж по отклику на заявку {name} ({order_id}) *подтвержден администратором*.\nМожно приступать к исполнению заказа, не забывай менять его статус в разделе *«Мои отклики»* - это необходимо сделать после приобретения товара и после его отправки.\n\n*Инструкция* по использованию сервиса содержится в разделе *"помощь/справка"*.'
...
def sended_product(order_id, name):
    ...
    return f'Исполнитель отметил товар по заявке {name} ({order_id}) как *отправленный*, ожидаем подтверждение получения с твоей стороны.\n\nПосле получения заказа тебе *необходимо* изменить его статус - *инструкция* по использованию сервиса содержится в разделе *"помощь/справка"*.'
...
RESPONSE_COMMENT = 'Укажи *комментарий к отклику*, воспользуйся командой */skip* для пропуска этого шага.' + CANCEL_RESPONSE_COMMAND
...
CARD = 'Введи *номер банковской карты* для совершения перевода.\nВ формате: *5555 4444 3333 2222*.' + CANCEL_RESPONSE_COMMAND
